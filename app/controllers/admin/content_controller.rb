...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    ...
    if @article.present? && @other_article.present?
      if @article.merge_with!(@other_article)
        redirect_to action: "index"
        return
      else
        flash[:error] = _("Error: Failed merge of article ##{@article.id} with ##{@other_article.id}")
      end
    else
      flash[:error] = _("Error: Failed merge of articles")
      redirect_to action: "index"
    end
  end
